...
def connect_db():
    ...
    alchemyEngine = create_engine('postgresql+psycopg2://kamal:kamal%401991@192.168.1.96:5432/donation-to-re', pool_recycle=3600)
...
def get_uploaded():
    logging.info('Identifying donations that are uploaded in RE')

    db = pd.read_sql_query('SELECT * FROM uploaded;', db_conn)
    return db


def get_missing_donations():
    logging.info('Identifying missing donations')
...
try:
    ...
    if new_donations.shape[0] > 0:
        ...
        for index, new_donation in new_donations:
            # Locate Donor

            # Create donor if not present
            # Upload Donations
            # Upload Receipts
