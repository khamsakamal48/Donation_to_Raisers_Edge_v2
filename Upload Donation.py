...
def upload_donation(df, const_id):
    ...
    params = {
        ...
        'receipts': [{
            'amount': {
                'value': 0 if df['totalamount'] < 0 else df['donationamount'] if pd.isnull(df['totalamount']) else df['totalamount']
            },
    ...
    if new_address:
        ...
        if process.extractOne(new_address, re_address_list)[1] >= 95:
            ...
            if country != '' or ~(country == 'India' and city == '' and state == ''):
                ...
                if country in ['Mauritius', 'Switzerland', 'France', 'Bahrain', 'Denmark', 'Netherlands', 'Singapore']:
                    state = ''
